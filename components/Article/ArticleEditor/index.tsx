...
const ArticleEditor = ({ defaultValues, _id, loading }: Props): JSX.Element => {
  ...
  if (query?._id) {
    useFormPersist({ name: query?._id, watch, setValue });
  } else {
    useFormPersist({ name: "createPost", watch, setValue });
  }
